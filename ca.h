...
public:
  ...
  bool gradientfield(int x, int y);

  float concentration[500][500];

  void PlotChem (Graphics *g);//plots the chemotaxant

  void concentration_(); //secretes chemotaxant at the border of the cell

  void clean_edges(); //"pushes" the chemotaxant that is inside a wall back into the medium

  void diffuse();//numerical diffusion


  std::unordered_set<signed long long int> alivePixels;
  bool AnyPillar();
