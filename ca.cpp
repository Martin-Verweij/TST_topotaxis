...
void CellularPotts::PlotChem(Graphics *g){ //plotting the chemotaxant
  float insty = 15;
  if (g) {
  for ( int i = 0; i < sizex-2;i++ )
    for ( int j = 0; j < sizey-2; j++ ) {
      if (concentration[i][j] != 0){
        g->Point(abs(insty*concentration[i][j]),2*i,2*j);
        g->Point(abs(insty*concentration[i][j]),2*i+1,2*j);
        g->Point(abs(insty*concentration[i][j]),2*i,2*j+1);
        g->Point(abs(insty*concentration[i][j]),2*i+1,2*j+1);
      }
    }
  } else return;
}

void CellularPotts::concentration_()
{
  int i, j,q;
...
void CellularPotts::clean_edges(){
    for (int i = 2; i < sizex-2; i++){
      for (int j =2; j < sizey-2; j++){
        if ( IsPillar(i,j) && concentration[i][j] != 0 ){
          if ( !IsPillar(i+1,j) ){
            concentration[i+1][j] += concentration[i][j];
          }
          else if (!IsPillar(i,j+1)){
            concentration[i][j+1] += concentration[i][j];
          }
          else if (!IsPillar(i-1,j)){
            concentration[i-1][j] += concentration[i][j];
...
void CellularPotts::diffuse(){
  float dt = 0.01;
  float uxx,uyy;
  for (int i =2; i < sizex-2 ; i++){
    for (int j =2; j < sizey-2 ; j++){
        uxx = concentration[i+1][j] - 2*concentration[i][j] + concentration[i-1][j];
        uyy = concentration[i][j+1] - 2*concentration[i][j] + concentration[i][j-1];
        concentration[i][j] += dt * (uxx + uyy);
    }
  }
  clean_edges();
  return;
        ...
        if ((p=CopyvProb(D_H,H_diss))>0) {
          ...
          for (int j = 1; j <= n_nb; j++){
    				...
    				if (xn>0 && yn>0 && xn<sizex-1 && yn<sizey-1){//if the neighbour site is within the lattice
    					if (edgelist[edgeadjusting] == -1 && sigma[xn][yn] != sigma[x][y] && sigma[xn][yn]+sigma[x][y]!=-2){ //if we should add the edge to the edgelist, add it
    						AddEdgeToEdgelist(edgeadjusting);
    						loop += (double)2/n_nb;
    					}
    					if (edgelist[edgeadjusting] != -1 && (sigma[xn][yn] == sigma[x][y] || sigma[xn][yn]+sigma[x][y]==-2)){//if the sites have the same celltype and they have an edge, remove it
            ...
            std::unordered_set<signed long long>::const_iterator it =(alivePixels.find((signed long long)(x*100000LL+y)));
            if (it==alivePixels.end()){
            alivePixels.insert((signed long long)(x*100000LL+y));
            }
          }
            else{
              PDEfield->setValue(2, x, y, 0);
              std::unordered_set<signed long long>::const_iterator it =(alivePixels.find((signed long long)(x*100000LL+y)));
              if (it!=alivePixels.end()){
              alivePixels.erase((signed long long)(x*100000LL+y));
              }
            }
          }
...
void **CellularPotts::PlotVectors(Graphics *g){
  //Edition for vector based persistence
  ...
  for (int i=1;i<=cell->size()-1;i++){
    ...
    double celltheta=(*cell)[i].theta;
    if (x1>sizex or x1<0){
        cout << "x1 outside field" << endl;
      }
    x2=x1+(int)(10*(std::cos(celltheta)));
    y2=y1+(int)(10*(std::sin(celltheta)));
    // cout << "add line" << endl;
...
bool CellularPotts::IsPillar(int x, int y){
  ...
  switch (par.wall_layout){
    case 1: {
      if (((abs(x-250)) <= par.box_size) && ((abs(y-250)) <= par.box_size))  return(false);
      else return(true);
    }
    case 2:{
      return gradientfield(x,y);
    }
  }
  return true;
}
bool CellularPotts::gradientfield(int x, int y){

  if (par.pillar_radius<=0)
    return(false);
  //check if there is a gradient
  if (par.pillar_r>0){
    // if((double)x-sizex/2.0>(2.1*par.pillar_radius-par.pillar_distance)/(1-exp(-par.pillar_r))+par.pillar_distance/2.0-par.pillar_radius &&
    // (double)x-sizex/2.0<(par.pillar_distance-2.1*par.pillar_radius)/(1-exp(-par.pillar_r))+par.pillar_distance/2.0+par.pillar_radius){
    double d_min=2.0*par.pillar_radius+6;
    double d_max=2*par.pillar_distance-d_min;

    double n=log(((double)x-(double)sizex/2.0-par.pillar_distance/2)*(1-exp(-par.pillar_r))/par.pillar_distance+1)/par.pillar_r;
    double n_low=floor(n);
    double n_high=ceil(n);
    double m_l=((double)y-(double)sizey/2.0)/(par.pillar_distance*exp(par.pillar_r*n_low))-1/2;
    double m_h=((double)y-(double)sizey/2.0)/(par.pillar_distance*exp(par.pillar_r*n_high))-1/2;
    double n_min=(1.0/par.pillar_r)*log((2.0*par.pillar_radius+6.0)/par.pillar_distance);
    double n_max=(1.0/par.pillar_r)*log(2.0-(2.0*par.pillar_radius+6.0)/par.pillar_distance);
    double n_min_star=ceil(n_min);
    double n_max_star=floor(n_max);
    double x_n_min_star=par.pillar_distance*(exp(par.pillar_r*n_min_star)-1)/(1-exp(-par.pillar_r))+par.pillar_distance/2.0;
    double x_n_max_star=par.pillar_distance*(exp(par.pillar_r*n_max_star)-1)/(1-exp(-par.pillar_r))+par.pillar_distance/2.0;
...
//n_low>=n_min && n_low<n_max &&
    if ( sqrt(pow((exp(par.pillar_r*n_low)-1)*par.pillar_distance/(1-exp(-1*par.pillar_r))+par.pillar_distance/2.0-(double)x+(double)sizex/2.0,2)
    +pow(par.pillar_distance*(floor(m_l)+1.0/2.0)*exp(n_low*par.pillar_r)-y+(double)sizey/2.0,2))<par.pillar_radius)
      return(true);
      //n_low>=n_min && n_low<n_max &&
    else if (sqrt(pow((exp(par.pillar_r*n_low)-1)*par.pillar_distance/(1-exp(-1*par.pillar_r))+par.pillar_distance/2.0-x+sizex/2.0,2)
    +pow(par.pillar_distance*(ceil(m_l)+1.0/2.0)*exp(n_low*par.pillar_r)-y+sizey/2.0,2))<par.pillar_radius)
      return(true);
      //n_high<n_max &&
    else if (sqrt(pow((exp(par.pillar_r*n_high)-1)*par.pillar_distance/(1-exp(-1*par.pillar_r))+par.pillar_distance/2.0-x+sizex/2.0,2)
    +pow(par.pillar_distance*(floor(m_h)+1.0/2.0)*exp(n_high*par.pillar_r)-y+sizey/2.0,2))<par.pillar_radius)
      return(true);
      //n_high<n_max &&
    else if (sqrt(pow((exp(par.pillar_r*n_high)-1)*par.pillar_distance/(1-exp(-1*par.pillar_r))+par.pillar_distance/2.0-x+sizex/2.0,2)
    +pow(par.pillar_distance*(ceil(m_h)+1.0/2.0)*exp(n_high*par.pillar_r)-y+sizey/2.0,2))<par.pillar_radius)
      return(true);
    // else
    //   return(false);
}
      ...
      if (n_low<n_min){
        ...
        //par.pillar_distance/2.0+(2.1*par.pillar_radius-par.pillar_distance)/(1-exp(-par.pillar_r));
      }
      if (n_high>n_max){
        pillar_distance=d_max;
        shift=x_n_max_star+d_max/2.0;
        //par.pillar_distance/2.0+par.pillar_distance/2.0+(par.pillar_distance-2.1*par.pillar_radius)/(1-exp(-par.pillar_r));
      }
      ...
      double new_n_low=floor(new_n);
      double new_n_high=ceil(new_n);
      double m_low=floor(m);
      double m_high=ceil(m);
      // cout << "n "<< n<<" m " << m<< endl;
